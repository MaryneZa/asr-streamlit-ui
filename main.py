...
def save_edited_csv(data_frame, csv_path, selected_set):
    """
    ...
    try:
        ...
        upload_edited_csv_file(edited_csv, csv_path, selected_set)
...
def handle_next_page():
    ...
    try:
        ...
        save_edited_csv(t_df, f"{csv_name}/train.csv", st.session_state.selected_set)
        save_edited_csv(v_df, f"{csv_name}/val.csv", st.session_state.selected_set)
...
def main():
    ...
    with cols_file[1]:
        if selected_csv_file:
            ...
            group_set = list(range(1, highest_value + 1))

            # Generate the list of options with labels indicating whether they are done or not
            options = [
                (num, f"{num} (done)") if num in done_set else (num, num) for num in group_set
            ]
